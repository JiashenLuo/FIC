...
class Tools:
    ...
    def random_walk(start_node, walk_length, adj_table, with_neg_edges = True):
        '''返回节点路径和路径类型，例如：
        [[0, 725, 1], [725, 3354, 1], [3354, 134, 5]]
        这代表三条路径，其中[0, 725, 1]表示起点为0，终点为725，路径类型为1
        路径类型的求解方式：二进制"01011"转十进制,其中0表示负边，1表示正边
        '''
        path = [[start_node]]
        path_types = []
        for i in range(walk_length):
            ...
            if last_node not in adj_table:# 路径最后的节点没有邻居
                return None
    ...
    def random_walk_sigformer(start_node, walk_length, adj_table, neg_adj_table):
        ...
        for i in range(walk_length):
            ...
            # 二进制边类型
            p_or_n = random.randint(0, 1) # positive or negative edge
            path_type = path_types[-1] + p_or_n * 2**i if path_types else p_or_n
