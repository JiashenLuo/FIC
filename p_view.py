...
def rss_show_list_sr(srs):
    ...
    if len(set_sta) == 1: # 单超参
        ...
        for sr in srs:
            ...
            print("[TimeStamp]", sr.timestamp)
            print("[Model Path]", sr.model_path)
...
def delete_model():
    ...
    def show_statistic(set_recs):
        ...
        print("**"*10,f"剩余记录数{len(set_recs)}","**"*10)
        pprint.pprint(statistices, compact=True, width=120)
        print("**"*20)
        
    while 1:
        show_statistic(all_rec_set)
        command = input("继续过滤（1），退出（quit），重置（0），删除（delete）")
        if command == "quit":
            break
        elif command == '1':
            filters = input("请输入过滤key value对（如：model_name bpr data_name ml_100k）")
            filters = filters.split(' ')
            
            # 列表转字典，奇数位置为参数名，偶数位置为参数值
            command_d = dict(zip([x for x in filters[::2]], [x for x in filters[1::2]]))
...
def train_curve():
    ...
    model_names = input("请输入模型名称(多个模型用空格分隔，输入all表示所有模型)：")
    ...
    for model_data, sr in sr_dict.items():
        print(f"model_data: {model_data} : ", round(sr.record.training_time_cost, 2))
        # 绘制性能指标
    ...
    # 保存数据
    import json
    print("Save training data to json file: ", f"visual_data/5_{data_name}.json")
