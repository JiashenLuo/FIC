...
from settings import Model_PARAMS_TABLE
from matplotlib.figure import Figure
import pprint
import os
from utils.recorder import MAX_STAGNANT_TIMES
    ...
    def Dict_Hash(d:dict)->int:
        ...
        for k, v in d.items():
            if isinstance(v, dict):
                res += hash(k) + Setting.Dict_Hash(v)
            else:
                res += hash(k)+hash(v)
        return res
...
class SettingFactory:
    ...
    def Get_Setting(setting_dict:dict) -> Setting:
        # 获取模型参数名
        model_p = Model_PARAMS_TABLE[setting_dict['model_name']]
        # 获取模型名，数据名
