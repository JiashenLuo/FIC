...
class Trainer:
    ...
    def Get_Train_Loader(self, data:datas.data.DataBasic):
        if self.params.model_name in ("dns", "dnsgcl", "dnsxsimgcl", 'ssm', 
                                      'cft', "cftcl", "cftcl1", "fic",
                                      "dnscl", "ahns", "dnsmn", 'cuco'):
...
class ModelDataParameterFactor:
    ...
    def get_model_params(data: datas.data.DataBasic, 
        ...
        if params.model_name in ['bpr', 'dns', "directau"]:
        ...
        elif params.model_name in ['lightgcn', "simgcl", "xsimgcl", "tag_cf", 
                                   'dnsgcl', "dnsxsimgcl", 'transgnn', "ssm", "fic", "dnscl",
                                   "ahns", "dnsmn", 'cuco']:
            return (data.user_num, data.item_num, data.Load_A_norm(), 
                    params)
        elif params.model_name in ['cpsvd']:
            return (params.embs_dim, data.user_num, params.hidden_num, 
                    params.individuality, data.Load_R(), data.Load_R_norm())
        elif params.model_name in ['sgformer']:
            return (data.user_num, data.item_num, data.edges, data.Load_A_norm(), 
                    params)
        elif params.model_name in ['lightgcl']:
            return (data.user_num, data.item_num, data.Load_R_norm(), 
                    params)
            ...
            return (data.user_num, data.item_num, 
                    ...
                    params)
        elif params.model_name in ['cftcl']:
            return (data.user_num, data.item_num,
                    data.pathes_ptypes(params.walk_len, params.path_num, 
                                       params.with_neg_edges),
                    params)
        elif params.model_name in ['cftcl1']:
            return (data.user_num, data.item_num,
                    data.pathes_ptypes(params.walk_len, params.path_num, params.with_neg_edges),# view 1
                    data.pathes_ptypes(params.walk_len, params.path_num1, params.with_neg_edges),# view 2
                    params)
