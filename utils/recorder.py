...
class Recorder:
    ...
    def Epoch_Record(self, epoch:int, loss_dict:dict):
        ...
        self.Dict_Show(loss_dict) # 格式化输出当前损失
        self.Record_Loss(loss_dict) # 损失记录
    ...
    def Performance_Record(self, epoch:int, eval_dict:dict)->int:
        '''输出模型指标，记录模型指标，判断是否需要早停
        '''
        self.Epoch_Print(epoch)
        self.Dict_Show(eval_dict) # 格式化输出
